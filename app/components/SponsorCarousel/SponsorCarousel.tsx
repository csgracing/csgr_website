...
function SponsorCarousel() {
  return (
      <div className="logos overflow-hidden whitespace-nowrap bg-black border-csg-red border-t-4 border-b-4 text-center py-1">
        ...
        <div className='flex z-0'>
            ...
            <div className="flex items-center justify-center-3 md:gap-x-2">
              {SponsorCarouselLogos.map((Logo,index) => (
                <a href={Logo.link} key={index} target="_blank" rel="noopener noreferrer">
                  <Logo.src className='w-auto h-20 md:h-30 fill-white'/>
                </a>
              ))}
            </div>
          </div>
          ...
          <div className="logos-slide">
            <div className="flex items-center justify-center-3 md:gap-x-2">
              {SponsorCarouselLogos.map((Logo,index) => (
                <a href={Logo.link} key={index} target="_blank" rel="noopener noreferrer">
                  <Logo.src className='w-1/8 h-20 md:h-30 fill-white'/>
                </a>
              ))}
            </div>
          </div>
